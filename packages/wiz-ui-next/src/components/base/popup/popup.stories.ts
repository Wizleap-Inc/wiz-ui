...
const Template: StoryFn<typeof WizPopup> = (args) => ({
...
const MultipleTemplate: StoryFn<typeof WizPopup> = (args) => ({
  ...
  setup() {
    ...
    return { isOpenIndex, changeIsOpenIndex, args };
  },
  template: `
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10rem;">
      <wiz-popup-container v-for="(pattern, i) in args.patterns" :key="pattern[args.main]" :value="isOpenIndex === i" @input="changeIsOpenIndex(i)">
        <wiz-text-button @click="changeIsOpenIndex(i)">Toggle Popup {{ pattern[args.main] }}</wiz-text-button>
...
export const MultiplePlayground: StoryFn<typeof WizPopup> = (args) => ({
  components: { WizPopup, WizTextButton, WizPopupContainer },
  setup() {
    ...
    return { isOpen1, isOpen2, toggle1, toggle2, args };
  },
  template: `
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10rem;">
